...
// Fetch subjects for this year level and semester that the student is enrolled in
$subjects = [];
if ($year_level && $current_semester) {
    require '../../db.php';
    $user_id = $_SESSION['user']['id'];
    
    // Modified query to only show subjects the student is enrolled in
    $stmt = $conn->prepare("SELECT s.id, s.name 
                           FROM subjects s 
                           JOIN subject_students ss ON s.id = ss.subject_id 
                           WHERE s.year_level = ? AND s.semester = ? AND ss.student_id = ? 
                           ORDER BY s.name");
    $stmt->bind_param("isi", $year_level, $current_semester, $user_id);
    $stmt->execute();
         ...
         <div class="row">
            ...
            <?php if (!empty($subjects)): ?>
                <?php foreach ($subjects as $subject): ?>
                    <a href="student-subject.php?id=<?php echo $subject['id']; ?>" class="btn btn-outline-secondary text-start fs-5 mb-3"><?php echo htmlspecialchars($subject['name']); ?></a>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="alert alert-info">You are not part of the classlist uploaded this semester.</div>
            <?php endif; ?>
        </div>
    </div>
