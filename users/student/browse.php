...
$sql = "SELECT r.*, u.name as uploader_name, s.name as subject_name, s.id as subject_id
        FROM resources r 
...
// Function to check if user can download a resource
function can_download_resource($resource, $user_id, $user_role) {
    // Admins can download everything
    if ($user_role === 'admin') {
        return true;
    }
    
    // Uploaders can download their own resources
    if ($resource['uploader_id'] == $user_id) {
        return true;
    }
    
    // Students and other users can only download if the uploader is an admin
    $uploader_role = get_user_role($resource['uploader_id']);
    return $uploader_role === 'admin';
}
