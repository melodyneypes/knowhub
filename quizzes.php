...
$quizzes = [];
$stmt = $conn->prepare("SELECT id, title, description, file_path, created_at, due_date FROM quizzes WHERE subject_id = ? ORDER BY created_at DESC");
$stmt->bind_param("i", $subject_id);
$stmt->execute();
...
// Handle quiz deletion (for instructors)
if ($is_instructor && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_quiz'])) {
    $quiz_id = intval($_POST['quiz_id']);
    
    // First get file path to delete the file
    $stmt = $conn->prepare("SELECT file_path FROM quizzes WHERE id = ? AND subject_id = ?");
    $stmt->bind_param("ii", $quiz_id, $subject_id);
    $stmt->execute();
    ...
    if ($stmt->fetch()) {
        ...
        $stmt->close();
        $delete_stmt = $conn->prepare("DELETE FROM quizzes WHERE id = ? AND subject_id = ?");
        $delete_stmt->bind_param("ii", $quiz_id, $subject_id);
        $delete_stmt->execute();
        $delete_stmt->close();
        
        // Redirect to refresh the page
        header("Location: quizzes.php?subject_id=$subject_id");
        exit();
    }
    $stmt->close();
}

// Handle quiz upload (for instructors)
$upload_message = '';
if ($is_instructor && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['quiz_file'])) {
    ...
    if (empty($title)) {
        ...
        $file_path = NULL;
        if (isset($_FILES['quiz_file']) && $_FILES['quiz_file']['error'] === UPLOAD_ERR_OK && is_uploaded_file($_FILES['quiz_file']['tmp_name'])) {
            $upload_dir = __DIR__ . '/uploads/quizzes/';
            ...
            if (!move_uploaded_file($_FILES['quiz_file']['tmp_name'], $file_path)) {
        ...
        if (empty($upload_message)) {
            // Save to database
            $stmt = $conn->prepare("INSERT INTO quizzes (subject_id, title, description, file_path, due_date, created_at) VALUES (?, ?, ?, ?, ?, NOW())");
            $stmt->bind_param("issss", $subject_id, $title, $description, $file_path, $due_date);
            $stmt->execute();
            $stmt->close();
            
            $upload_message = "<div class='alert alert-success'>Quiz uploaded successfully.</div>";
...
<head>
    <meta charset="UTF-8">
    <title>Quizzes - <?php echo htmlspecialchars($subject_name); ?></title>
...
<div class="container mt-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <h2 class="mt-3">Quizzes for <?php echo htmlspecialchars($subject_name); ?></h2>
        </div>
        <?php if ($is_instructor): ?>
            <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#uploadModal">+ Add Quiz</button>
        <?php endif; ?>
    </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <?php if (!empty($quiz['due_date'])): ?>
                        <p><strong>Due Date:</strong> <?php echo date('M j, Y', strtotime($quiz['due_date'])); ?></p>
                    <?php endif; ?>
                    <?php if (!empty($quiz['file_path']) && file_exists($quiz['file_path'])): ?>
                        <a href="<?php echo htmlspecialchars($quiz['file_path']); ?>" class="btn btn-primary" download>Download Quiz File</a>
                    <?php else: ?>
    ...
    <?php else: ?>
        <div class="alert alert-info">
            ...
            <p>There are currently no quizzes for this subject.</p>
            <?php if ($is_instructor): ?>
                <hr>
                <p class="mb-0">As an instructor, you can add quizzes using the "Add Quiz" button above.</p>
            <?php endif; ?>
        </div>
    <?php endif; ?>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="uploadModalLabel">Upload New Quiz</h5>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="quiz_file" class="form-label">File (optional)</label>
                        <input type="file" class="form-control" id="quiz_file" name="quiz_file">
